...
def main():
    ...
    with open(os.path.join('datasets', 'vicuna-all.json'), 'w') as all_output_file:
        json.dump(new_data, all_output_file, indent=2)

    with open(os.path.join('datasets', 'vicuna.short.json'), 'w') as short_output_file:
        json.dump(filtered_data, short_output_file, indent=2)
